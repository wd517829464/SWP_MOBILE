...
export default class AreaScreen extends Component {
    ...
    callbackParent(type){
        if(type=='confirm'){
            var areaCode=this.state.countryCode?this.state.countryCode:(this.state.cityCode?this.state.cityCode:this.state.provinceCode);
            var cityName='';
            if(areaCode.substring(2,6)=='0000'){//省
                cityName=ChinaCity.provinces[areaCode];
            }else if(areaCode.substring(4,6)=='00'){//市
                var provinceCode=areaCode.substring(0,2)+'0000';
                cityName=ChinaCity[provinceCode][areaCode];
            }else{//区
                var provinceCode=areaCode.substring(0,2)+'0000';
                var cityCode=areaCode.substring(0,4)+'00';
                cityName=ChinaCity[cityCode][areaCode];
            }
            this.props.callbackParent({
                action:'confirm',
                param:{areaCode:areaCode,areaName:cityName}
            });
        }else{
            this.props.callbackParent({
                action:'cancel',
    ...
    render() {
        ...
        <View style={styles.Dialog} width={widths} height={heights} top={0}>
            <View style={styles.mytop}>
                <View></View>
                <View style={styles.middleView}><Text style={styles.middleText}>选择行政区</Text></View>
                <View></View>
            </View>

                    ...
                    option:(NIMKitInfoFetchOption *)option
{
    ...
    switch (sessionType) {
        ...
        case NIMSessionTypeTeam:
        {
            ...
            //修改头像地址https的策略，单独修改，不修改云信整体配置 0418 zl
            NSString *orginUrl = @"";
            NSArray *tmpArr = [userInfo.avatarUrl componentsSeparatedByString:@":"];
            if(tmpArr && [tmpArr firstObject]){
                if([tmpArr lastObject]){
                    orginUrl = [@"http:" stringByAppendingString:[tmpArr lastObject]];
                }
            }
            info.avatarUrlString = orginUrl;//  原本： userInfo.thumbAvatarUrl;//0418 修改头像地址
            info.avatarImage = [UIImage imageNamed:@"default-placeholder-icon.png"]; //self.defaultUserAvatar;//
